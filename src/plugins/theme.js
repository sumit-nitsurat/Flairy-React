			...
			plugins : function(){
				...
				if($(pluginsCollection[6]).length){
					...
					var callback = function (playerShots) {
			            ...
			            $(pluginsCollection[6]).php(html).find('.jackbox[data-group]').jackBox("newItem", {
					        group: "dribbble"
					    });
			        };
					...
					var carousel = $(pluginsCollection[8]),
						...
						carousel.flexslider({
							...
							start: function(){
								...
								button.eq(0).php('<i class="icon-angle-up"></i>').parent().addClass('f_left m_right_5')
								button.eq(1).php('<i class="icon-angle-down"></i>').parent().addClass('f_left');
							}
						});
			...
			helpers : {
				...
				getNewRandomElement : function (filterClasses,container,classN){
					...
					element.addClass(filterClasses[Core.helpers.getRandom(0,filterClasses.length)])
							.php(otherElem.eq(Core.helpers.getRandom(0,otherElem.length)).php());
				...
				newsLetter : function(){
					...
					subscribe.on('submit',function(e){
						...
						if(self.find('input[type="email"]').val() == ''){
							...
							message.php('<div class="alert_box error r_corners relative fs_medium"><p>'+text+'</p></div>')
								.addClass('opened')
								.slideDown()
								.delay(4000)
								.slideUp(function(){
									$(this).php("").removeClass('opened');
								});
							...
							$.ajax({
								...
								success: function(data){
									if(data == '1'){
										...
										message.php('<div class="alert_box success r_corners relative fs_medium"><p>'+text+'</p></div>')
											.addClass('opened')
											.slideDown()
											.delay(4000)
											.slideUp(function(){
					...
					cf.on("submit",function(event){
						...
						request.then(function(data){
							...
							if(data == "1"){
								...
								mcontainer.php('<div class="alert_box success r_corners relative fs_medium"><p>'+text+'</p></div>')
									.addClass('opened')
									.slideDown()
									.delay(4000)
									.slideUp(function(){
								...
								$('.message_container').php('<div class="alert_box error r_corners relative fs_medium"><p>'+text+'</p></div>')
									.addClass('opened')
									.slideDown()
									.delay(4000)
									.slideUp(function(){
							...
							$('.message_container').php('<div class="alert_box error r_corners relative fs_medium"><p>Connection to server failed!</p></div>')
									.addClass('opened')
									.slideDown()
									.delay(4000)
									.slideUp(function(){
